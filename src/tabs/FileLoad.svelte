<script lang="ts">
	...
	import ZipWorker from '../lib/worker/Zip?worker&inline';
	...
	function load() {
		...
		if (inputPdf.length > 0) {
			if (inputPdf[0].type === 'application/x-zip-compressed') {
				QrScanner.createQrEngine().then((qrWorker) => {
					...
					worker.onmessage = (e) => {
						...
						if (data.type === 'unzip') {
							...
							for (let i = 0; i < datas.length; i++) {
								QrScanner.scanImage(datas[i], { returnDetailedScanResult: true, qrEngine: qrWorker }).then((qr) => {
...
<div class="root">
	<div>
		...
		<label for="file" on:mouseenter={mouseenter} on:mouseleave={mouseleave} class="fileLabel">
			<FileIcon size="60" class="animFile" />
			<p class="title">{zipLoading ? `분석중 ${complete} / ${allCount}` : '2. .zip/.otf 파일 업로드'}</p>
			<p class="detail">
				{zipLoading ? '완료되면 자동으로 페이지가 넘어갑니다' : '각 페이지를 jpg 또는 png로 변환하여 압축 / 기존 폰트 수정'}
			</p>
		</label>
		<input type="file" name="file" id="file" bind:files={inputPdf} accept=".zip,.otf" on:change={load} disabled={zipLoading} />
	</div>
